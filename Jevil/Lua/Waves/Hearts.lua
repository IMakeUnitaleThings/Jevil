...
function Update()
    ...
    if spawntimer%30 == 0 then
        ...
        local radius = 140
        local posx = radius*(math.cos(angle*math.pi/180))
        local posy = radius*(math.sin(angle*math.pi/180))

        local bullet = CreateProjectile('bullets/spr_heartbullet', posx, posy)
        bullet.SetVar('angle', angle)
        bullet.SetVar('radius', radius)
        bullet.SetVar('timer1', math.random(45, 60))
        bullet.SetVar('timer2', 10)
    ...
    for i=1,#bullets do
        ...
        if timer1 > 0 then 
            ...
            angle = angle + speed
            bullet.SetVar('angle', angle)
            local newposx = radius*(math.cos(angle*math.pi/180))
            local newposy = radius*(math.sin(angle*math.pi/180))
