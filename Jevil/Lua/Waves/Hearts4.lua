...
function Update()
    ...
    if spawntimer%20 == 0 then
        for i=1, 1, 2 do
            ...
            local radius = math.random(75, 120)
            local px = Player.x
            local py = Player.y
            local posx = px + radius*(math.cos(angle*(math.pi/180)))
            local posy = py + radius*(math.sin(angle*(math.pi/180)))
    ...
    for i=1,#bullets do
        ...
        local px = bullet.GetVar('px')
        local py = bullet.GetVar('py')

        if timer1 > 0 then 
            bullet.SetVar('timer1', timer1-1)
            angle = angle + speed
            bullet.SetVar('angle', angle)
            local newposx = px + radius*(math.cos(angle*math.pi/180))
            local newposy = py + radius*(math.sin(angle*math.pi/180))
            bullet.MoveTo(newposx, newposy)
            bullet.sprite.rotation = math.deg(math.atan2(py - bullet.y, px - bullet.x)) + 90
